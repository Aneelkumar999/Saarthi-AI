...
class AIService:
    ...
    async def parse_intent(self, db: Session, query: str):
        ...
        if self.llm:
            ...
            try:
                result = await chain.ainvoke({"query": query, "intent_descriptions": intent_descriptions})
                parsed = self._parse_json(result.content)
                return {"intent_id": parsed.get("intent_id"), "intent_name": parsed.get("intent_name") or "Unknown"}
    ...
    async def get_chat_response(self, db: Session, message: str, context: str = ""):
        ...
        if self.llm:
            prompt = ChatPromptTemplate.from_template(
                "You are Saarthi AI, an expert guide for Indian citizens. You help them navigate government services, licenses, and schemes.\n"
                "Your tone is professional, helpful, and empathetic.\n\n"
                "Context of available roadmaps: {intent_list}\n"
            ...
            chain = prompt | self.llm
            try:
                result = await chain.ainvoke({"message": message, "context": context, "intent_list": intent_list})
    ...
    async def generate_roadmap(self, db: Session, message: str):
        if self.gemini_model:
            ...
            prompt = f"""You are Saarthi AI, an expert Telangana government services navigator.
...
"""
    ...
    async def _build_roadmap_from_db(self, db: Session, message: str):
        ...
        for idx, isvc in enumerate(intent_services):
            ...
            status = "Ready" if idx == 0 else (f"Blocked by Step {idx}" if dep_ids else "Pending")
            steps.append({
                "id": idx + 1, "title": svc.name, "dept": svc.department,
                "status": status, "days": f"{max(1, svc.sla_days // 2)}-{svc.sla_days} days",
        ...
        return self._normalize_roadmap({
            "intent": best_match.name, "location": location,
            "title": f"{best_match.name} roadmap for {location}",
            "timeline": f"{max(5, total_days // 2)}-{total_days + 5}",
    ...
    def _get_schemes_for_intent(self, db: Session, intent):
        ...
        matched = []
        keyword_map = {
            "tea shop": ["vendor", "business"], "birth certificate": ["general"],
            "farm subsidy": ["farmer"], "marriage": ["general"], "property": ["general"],
            "msme": ["business"], "caste": ["sc_st"], "income": ["bpl"],
            "driving": ["general"], "voter": ["general"], "pan": ["general"],
            "passport": ["general"], "trade": ["vendor", "business"],
            "pollution": ["business"], "fire": ["business"], "building": ["business"],
            "ration": ["bpl"], "senior": ["general"],
        }
    ...
    def _hardcoded_fallback(self, message: str):
        return self._normalize_roadmap({
            "intent": "Government service request", "location": "Telangana",
            "title": "Government service roadmap", "timeline": "7-15",
    ...
    def _normalize_roadmap(self, data: dict, goal: str):
        ...
        if not normalized_steps:
            normalized_steps = [{
                "id": 1, "title": "Service eligibility and document check",
                "dept": "MeeSeva / Relevant department", "status": "Ready",
                "days": "1-3 days", "documents": ["Aadhaar", "Address proof"]
            }]
